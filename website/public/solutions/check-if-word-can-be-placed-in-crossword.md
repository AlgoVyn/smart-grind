...
Time complexity: O(m * n * 4 * len(word)), as we check up to 4 directions per cell, and for each, iterate through the word length. Since len(word) <= max(m, n) and m*n <= 2e5, it's efficient. Space complexity: O(1), excluding the input.

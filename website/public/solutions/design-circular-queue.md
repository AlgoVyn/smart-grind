...
**Time Complexity:** All operations (`enQueue`, `deQueue`, `Front`, `Rear`, `isEmpty`, `isFull`) are O(1) because they involve constant-time array accesses and arithmetic operations.

**Space Complexity:** O(k), where k is the maximum size of the queue, as we use a fixed-size array of size k+1.

...
**Time Complexity:** O(N + E), where N is the number of recipes and E is the total number of ingredients across all recipes, as we process each recipe and ingredient once.

**Space Complexity:** O(N + E), for the graph, indegree map, queue, and result list.

...
We use a stack to keep track of surviving asteroids. For each asteroid:
- While the stack has a right-moving asteroid (positive) and the current is left-moving (negative), simulate collision.
- If the stack top is smaller than the current's size, pop it and continue checking.
- If equal, both destroy, pop and don't add current.
- If stack top is larger, current destroys, don't add it.

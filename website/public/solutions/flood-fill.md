...
Flood fill is a graph traversal problem where we change the color of all connected pixels of the same original color starting from a given pixel. We use DFS or BFS to explore all adjacent pixels (up, down, left, right) that match the original color, changing them to the new color.
...
**Time Complexity:** O(m * n), as we may visit each pixel at most once.

**Space Complexity:** O(m * n), in the worst case for the recursion stack or queue.

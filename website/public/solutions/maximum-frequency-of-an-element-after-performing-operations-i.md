...
First, sort the array `nums` to enable efficient range queries using binary search.

Use a `Counter` to store the frequency of each number in `nums`.

For each unique number `num` in `nums` (potential target), perform the following:
...
**Time Complexity:** O(n log n) due to sorting and binary search operations for each unique element.
**Space Complexity:** O(n) for the counter and sorted array.

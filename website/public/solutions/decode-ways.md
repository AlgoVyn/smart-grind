...
- If the current character `s[i-1]` is not '0', we can take it as a single digit, so add `dp[i-1]`.
- If the last two characters form a number between 10 and 26, we can take them as a two-digit code, so add `dp[i-2]`.

The final answer is `dp[n]`.

Time complexity: O(n), where n is the length of the string, as we iterate through the string once.
Space complexity: O(n) for the dp array, but can be optimized to O(1) by using only two variables.

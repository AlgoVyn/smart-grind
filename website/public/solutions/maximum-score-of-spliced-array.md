...
1. **Compute sums:** Calculate the sums of `nums1` and `nums2`.

2. **Difference array:** The difference array `diff[i] = nums2[i] - nums1[i]` represents the gain when swapping element `i`.

3. **Kadane's algorithm:** Use Kadane's algorithm to find the maximum and minimum subarray sums of `diff`.
...
- **O(n)**, where `n` is the length of the arrays (due to Kadane's algorithm)

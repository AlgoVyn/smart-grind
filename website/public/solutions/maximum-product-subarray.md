...
1. **Initialize:** Set `max_prod`, `min_prod`, and `result` to the first element.

2. **Iterate Through Array:** For each subsequent number:
...
3. **Return Result:** The `result` holds the maximum product found

### Time Complexity

- **O(n)**, where `n` is the length of `nums` (single pass through the array)

...
1. **Sort Envelopes:**
   - Sort by width ascending. For equal widths, sort height descending to prevent nesting envelopes with same width.

2. **Extract Heights:**
   - Create a list of heights from the sorted envelopes.

3. **Compute LIS on Heights:**
   - Use a dynamic programming approach with binary search (patience sorting) to find the length of the longest increasing subsequence.
...
4. **Return Length:**
   - The length of `tails` is the maximum number of nested envelopes.

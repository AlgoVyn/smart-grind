...
First, build a map for nums2: value to its next greater element.

Use a stack on nums2: iterate, while stack not empty and nums2[stack[-1]] < nums2[i], pop and set map[nums2[popped]] = nums2[i]

Push i.

For nums1, for each value, if in map, use map[value], else -1.

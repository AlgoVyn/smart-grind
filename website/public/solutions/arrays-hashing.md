...
This approach ensures we detect the first duplicate encountered. Time complexity is O(N) since set operations are O(1) on average, and space complexity is O(N) in the worst case when all elements are unique.

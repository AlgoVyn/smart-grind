...
describe('Auth API', () => {
  ...
  describe('onRequestGet', () => {
    ...
    test('should return 400 for invalid code in callback', async () => {
      ...
      expect(await response.text()).toBe('Invalid code');
    });

    test('should return 400 for code too long', async () => {
    ...
    test('should return 400 for OAuth failure', async () => {
      mockFetch.mockResolvedValueOnce({ json: () => Promise.resolve({}) }); // No access_token
    ...
    test('should return 400 for invalid user data', async () => {
      ...
      expect(response.status).toBe(400);
      expect(await response.text()).toBe('Invalid user data');
    });

    test('should handle successful OAuth and return HTML for new user', async () => {
      mockFetch
        .mockResolvedValueOnce({ json: () => Promise.resolve({ access_token: 'token' }) })
        .mockResolvedValueOnce({ json: () => Promise.resolve({ id: 'user123', name: 'Test User', email: 'test@example.com' }) });
    ...
    test('should handle successful OAuth for existing user without storing again', async () => {
      mockFetch
        .mockResolvedValueOnce({ json: () => Promise.resolve({ access_token: 'token' }) })
        .mockResolvedValueOnce({ json: () => Promise.resolve({ id: 'user123', name: 'Test User' }) });
    ...
    test('should handle user without name', async () => {
      mockFetch
        .mockResolvedValueOnce({ json: () => Promise.resolve({ access_token: 'token' }) })
        .mockResolvedValueOnce({ json: () => Promise.resolve({ id: 'user123', email: 'test@example.com' }) });
    ...
    test('should handle user with no name or email', async () => {
      mockFetch
        .mockResolvedValueOnce({ json: () => Promise.resolve({ access_token: 'token' }) })
        .mockResolvedValueOnce({ json: () => Promise.resolve({ id: 'user123' }) });

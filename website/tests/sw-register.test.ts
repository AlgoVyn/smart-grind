...
describe('Service Worker Registration Module', () => {
    ...
    describe('CONTENT_UPDATE message handling', () => {
        test('should emit contentUpdate event when CONTENT_UPDATE message received', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
            const callback = jest.fn();
            const unsubscribe = on('contentUpdate', callback);

            // First, register the service worker to set up the message handler
            await registerServiceWorker();

            // Get the message handler from navigator.serviceWorker.addEventListener
            const serviceWorkerAddEventListener = (navigator.serviceWorker as any).addEventListener;
            const messageHandlerCall = serviceWorkerAddEventListener.mock.calls.find(
                (call: any[]) => call[0] === 'message'
            );
            const messageHandler = messageHandlerCall?.[1];

            if (messageHandler) {
                // Simulate CONTENT_UPDATE message from service worker
                // Note: event.data has type and data (payload) properties
                messageHandler({
                    data: {
                        type: 'CONTENT_UPDATE',
                        data: {
        ...
        test('should not emit event for unknown message type', async () => {
            const callback = jest.fn();
            const unsubscribe = on('contentUpdate', callback);
    ...
    describe('FORCE_RELOAD message handling with loop protection', () => {
        test('should skip reload when recently reloaded (within cooldown period)', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
            const callback = jest.fn();
            const unsubscribe = on('contentUpdate', callback);
            ...
            // Set recent reload timestamp (within 60 seconds)
            const recentReloadTime = (Date.now() - 30000).toString(); // 30 seconds ago
            mockSessionStorage.getItem.mockReturnValue(recentReloadTime);
        ...
        test('should trigger reload when no recent reload timestamp exists', async () => {
            const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
            const callback = jest.fn();
            const unsubscribe = on('contentUpdate', callback);
            ...
            // Set sessionStorage to return null (no recent reload)
            mockSessionStorage.getItem.mockReturnValue(null);

            // Note: Cannot spy on window.location.reload in jsdom due to read-only property
            // Instead, verify the console output and sessionStorage.setItem were called
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[SW] New content available, reloading...');
            // Should have set the reload timestamp
